...
_Table = prism' (\(a, b, c, d, e, f) -> Table a b c d e f) g
  where
    g (Table a b c d e f) = Just (a, b, c, d, e, f)
    g _ = Nothing
...
blockPrePlate f blk =
  case blk of
    ...
    Table attr caption colspec thead tbodies tfoot ->
      Table attr caption colspec
        <$> traverseOf (tableHeadRows . each . rowCells . each . cellBlocks) f thead
        <*> traverseOf (each . tableBodyRows . each . rowCells . each . cellBlocks) f tbodies
        <*> traverseOf (tableFootRows . each . rowCells . each . cellBlocks) f tfoot
...
tableHeadRows f (TableHead attr rows) = TableHead attr <$> f rows

tableFootRows :: Lens' TableFoot [Row]
tableFootRows f (TableFoot attr rows) = TableFoot attr <$> f rows

tableBodyRows :: Traversal' TableBody [Row]
tableBodyRows f (TableBody attr rowHeadCols rowsA rowsB) =
  TableBody attr rowHeadCols <$> f rowsA <*> f rowsB

rowCells :: Lens' Row [Cell]
rowCells f (Row attr cs) = Row attr <$> f cs

cellBlocks :: Lens' Cell [Block]
cellBlocks f (Cell attr alignment rowspan colspan blks) =
  Cell attr alignment rowspan colspan <$> f blks
